...
module.exports = function (getBase) {
  return function() {
    ...
    function addFaces(faces, className, numJitters = 0) {
      ...
      const promiseGetters = flatten(
        faces.map(f => getJitteredFaces(f, numJitters).map(() => () => getNet().computeFaceDescriptorAsync(f)))
      )
      const resultPromise = promiseGetters.reduce(
        (resPromise, getPromise) => resPromise
          .then(res => getPromise().then(desc => res.concat(desc))),
        Promise.resolve([])
      )
    ...
    function predict(face) {
      if (getDescriptorsByClass().length < 2) {
